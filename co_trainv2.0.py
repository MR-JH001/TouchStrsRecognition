...
def train(model, config, dataset_num: int = 1):
    ...
    for epoch in range(last_epoch, config.TRAIN.END_EPOCH):
        ...
        lr_scheduler.step()
        train_ls = []
        acc, loss = function.validate( config, val_loader, val_dataset, converter, model, criterion, device, epoch,
                                       writer_dict, output_dict )
...
def co_train_helper(model1, model2, model3, co_train_loader, config, device, converter):
    ...
    with torch.no_grad():
        for i, (inp, idx) in enumerate(co_train_loader):
            ...
            sim_preds1 = converter.decode_to_label(preds1.data, preds_size1.data, raw=False)

            preds_size2 = torch.IntTensor([preds2.size(0)] * inp.size(0))
            _, preds2 = preds2.max(2)
            preds2 = preds2.transpose(1, 0).contiguous().view(-1)
            sim_preds2 = converter.decode_to_label(preds2.data, preds_size2.data, raw=False)

            preds_size3 = torch.IntTensor([preds3.size(0)] * inp.size(0))
            _, preds3 = preds3.max(2)
            preds3 = preds3.transpose(1, 0).contiguous().view(-1)
            sim_preds3 = converter.decode_to_label(preds3.data, preds_size3.data, raw=False)

            for j in range(0, config.TRAIN.BATCH_SIZE_PER_GPU):
                j = int(j)
                inx = int(idx[j])
                ...
                res, string = compare(sim_pred1, sim_pred2, sim_pred3)
                if string:
                    # for test
...
def compare(preds1, preds2, preds3):
...
def add_label_helper(config, string, num: int, idx: int):
    ...
    curr_file = config.DATASET4.JSON_FILE['train'] + '/' + str(idx) + '.png'
    if num == 1:
