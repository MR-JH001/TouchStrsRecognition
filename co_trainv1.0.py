...
def co_train(config, model1, model2, model3, epoch):
    ...
    co_train_helper(model1, model2, model3, train_loader, config, device)
...
def co_train_helper(model1, model2, model3, co_train_loader, config, device):
    model1.eval()
    model2.eval()
    model3.eval()
    generated_label = []
    ...
    with torch.no_grad():
        for i, (inp, idx) in enumerate(co_train_loader):
            ...
            res, string = compare(preds1, preds2, preds3)
            if str:
                add_label_helper(config, string, res, idx)
            elif res == 4:
                continue
            else:
                corr += 1
...
def add_label_helper(config, string, num, idx):
