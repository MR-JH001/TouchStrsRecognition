...
class CRNN(nn.Module):
    ...
    def forward(self, input):
        ...
        input_to_craft = input #torch.Size([64, 3, 32, 100])
        score_link = input[0,  0,:, :].cpu().data.numpy()
        cv2.imwrite( "input.jpg", score_link * 255 )
        ...
        score_link = craft_y[0, :, :, 0].cpu().data.numpy()
        cv2.imwrite( "craft_y.jpg", score_link * 255 )
        craft_feature = craft_feature.permute( 0, 3, 1, 2 )
        conv = self.cnn(input)
        conv = F.interpolate( conv, size=(16,50), mode='bilinear',align_corners=False )
        score_link = conv[0, :, :, 0].cpu().data.numpy()
        cv2.imwrite( "conv1.jpg", score_link * 255 )
        score_link = craft_feature[0, :, :, 0].cpu().data.numpy()
        cv2.imwrite( "craft_feature.jpg", score_link * 255 )
        conv =  craft_feature + conv
        score_link = conv[0, :, :, 0].cpu().data.numpy()
        cv2.imwrite( "conv2.jpg", score_link * 255 )

        conv = F.interpolate( conv, size=(1, 26), mode='bilinear',align_corners=False )
        score_link = conv[0,  0, :, :].cpu().data.numpy()
        cv2.imwrite( "conv4.jpg", score_link * 255 )
        conv = conv.squeeze(2)
