...
class CRNN(nn.Module):
    ...
    def forward(self, input):
        ...
        craft_y = F.interpolate(craft_y, size=(craft_y.shape[-2]*4, craft_y.shape[-1]*512), mode='bilinear', align_corners=False )
        craft_y = craft_y[0, 0, :, :]
        craft_y = craft_y.unsqueeze(0)
        conv = self.cnn(input)

        print( "134.shape=", conv.shape )
        conv = conv[:, :, 0, 0]
        conv = conv.unsqueeze( 0 )
        conv = F.interpolate( conv, size=(conv.shape[2] * 25), mode='linear',
                                   align_corners=False )
        conv_Text = conv + craft_y.expand(26,64,25600)
        conv_Text = F.interpolate(conv_Text, size=[1024])
        conv_Text = conv_Text.expand( 26, 64, 1024 )
