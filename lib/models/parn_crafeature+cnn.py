...
class CRNN(nn.Module):
    ...
    def forward(self, input):
        ...
        craft_feature = craft_feature.permute( 0, 2, 3, 1 )#([16, 16, 100, 32])
        craft_feature = F.interpolate( craft_feature, size=(100, 1024), mode='bilinear', align_corners=False )
        craft_feature = craft_feature.permute( 0, 3, 1, 2 )#([16, 1024, 16, 100])
        conv = self.cnn(input)
        conv = F.interpolate( conv, size=(16, 100), mode='bilinear', align_corners=False )


        conv = craft_feature + conv
        conv = F.interpolate( conv, size=(1, 51), mode='bilinear', align_corners=False )
        conv = conv.squeeze(2)
